...
const Vote = () => {
  ...
  const handleVote = async () => {
    ...
    try {
      const token = await user.getIdToken();
      const response = await axiosInstance.post('/vote', { party, idToken: token });
      setMessage(response.data);
    } catch (error) {
      // Display detailed error message
      console.error('Error casting vote:', error.response || error.message);
      setMessage('Error: Could not cast vote. Please try again later.');
    }
  };
